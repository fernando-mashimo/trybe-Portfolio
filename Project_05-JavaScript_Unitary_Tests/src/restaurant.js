...
const createMenu = (menu) => {
  const obj = {
      fetchMenu: () => menu,
      consumption: [],
      order: (request) => obj.consumption.push(request), // Fernando's Annotation: object 'obj' created for the technical purpose of not inserting recursive function 'createMenu.consumption.push(request)' within 'createMenu' function.
      pay: () => {
        let sum = 0;

        for (let item of obj.consumption) { 
          if (menu.food[item]) { // Fernando's Annotation: this solution is a little bit more verbose, although more readable (for me, for the time being).
            sum += menu.food[item];
          } else sum += menu.drink[item];
        }
...
// Testes 1 e 2: console.log(Object.keys(createMenu({ food: {}, drink: {} }).fetchMenu()));
// Teste 3: console.log(createMenu({ 1: 'A', 2: 'B' }).fetchMenu());
// Teste 4: console.log(createMenu().consumption);
