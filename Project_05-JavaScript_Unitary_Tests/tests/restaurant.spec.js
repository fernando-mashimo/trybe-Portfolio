...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  const objectSample = { food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} };
  ...
  it('Verifique se função `createMenu()` retorna um objeto que possui a chave `fetchMenu`, a qual tem como valor uma função.', () => {
    const objetoRetornado = createMenu(objectSample);
    expect(Object.keys(objetoRetornado)).toEqual(['fetchMenu', 'consumption', 'order', 'pay']);
  ...
  // ```
  it('Verifique se o menu passado para função createMenu() é idêntico ao menu recuperado pela função "objetoRetornado.fetchMenu()"', () => {
    const objetoRetornado = createMenu(objectSample);
    expect(objectSample).toEqual(objetoRetornado.fetchMenu());
  });
